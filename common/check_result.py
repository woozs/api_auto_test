...
from common import check_json, expected_manage, custom_fail
...
def check(test_name, case_data, code, data, relevance, _path, success):
    ...
    # 校验json格式
        ...
        if isinstance(case_data["expected_request"], str):
                expected_request = expected_manage.read_json(test_name, expected_request, relevance, _path, success)
        ...
        if int(code) == case_data["expected_code"]:
            ...
            if case_data.get("CustomFail"):
                info = custom_fail.custom_manage(case_data.get("CustomFail"), relevance)
